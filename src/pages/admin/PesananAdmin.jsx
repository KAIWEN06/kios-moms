...
const PesananAdmin = ({
  ...
  const [uangDiterima, setUangDiterima] =
  useState('');
  ...
  useEffect(() => {
    ...
    cartItems.forEach((id) => {
      const currentMenu = menu.find((item) => Number(item.id) === Number(id));

      if (!currentMenu) {
        updateQty(Number(id), -999);
  ...
  let totalHarga = 0;
  cartItems.forEach((id) => {
    const m = menu.find((item) => Number(item.id) === Number(id));
    if (m) {
    ...
    <div className="p-3 sm:p-6 md:p-8 lg:p-10 bg-slate-50 min-h-screen text-slate-800 antialiased">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="hidden sm:inline-block text-xs font-semibold text-slate-400 bg-slate-100 px-3 py-1 rounded-md">
            Langkah 2: Checkout Kasir
          </span>
        </div>
                    ...
                    >
                      <div className="flex items-center gap-3.5 w-full sm:w-auto">
                        ...
                        <div className="min-w-0 flex-1">
                          <h5 className="font-bold text-slate-800 text-sm sm:text-base tracking-tight truncate">
                            {m.nama}
                          </h5>
        ...
        <div className="fixed inset-0 z-[999] bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white w-full max-w-[500px] rounded-2xl p-5 sm:p-6 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-2.5">
              <div>
                <h4 className="text-xl font-black text-[#002366]">Pilih Meja Kios</h4>
                <p className="text-xs text-slate-400 font-medium">Klik pada nomor meja yang masih kosong</p>
              </div>
              <button
                onClick={() => setShowMejaPopup(false)}
                type="button"
                className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-rose-500 hover:text-white text-slate-500 font-black transition-all flex items-center justify-center min-w-[32px] min-h-[32px]"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
              ...
              {Array.from({ length: 10 }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isUsed) {
                        toast.error(`Meja ${nomor} sedang digunakan`);
                        return;
                      }
                      setMeja(nomor);
                    }}
                    disabled={isUsed}
                    type="button"
                    className={`h-14 sm:h-16 rounded-xl font-black text-lg transition-all duration-200 min-h-[44px] active:scale-95 flex items-center justify-center border ${
                      selected
                        ? "bg-[#FF8C00] text-white border-[#FF8C00] shadow-sm scale-105"
                        : isUsed
                        ? "bg-slate-200 text-slate-400 border-slate-200 cursor-not-allowed text-sm line-through"
                        : "bg-white hover:bg-[#002366] text-slate-700 hover:text-white border-slate-200 hover:border-[#002366]"
                    }`}
                  >
                    {nomor}
                  </button>
            ...
            <button
              ...
              disabled={!meja}
              type="button"
              className="w-full mt-4 bg-[#002366] hover:bg-indigo-900 disabled:bg-slate-200 text-white disabled:text-slate-400 py-3 rounded-xl font-black text-sm transition-all min-h-[44px]"
            >
              Konfirmasi Meja {meja && `${meja}`}
            </button>
